...
def main():
    ...
    parser.add_argument("--app", default=os.environ.get("FIREBASE_APP_ID"), help="App bundle ID (default: $FIREBASE_APP_ID, dots converted to underscores)")
    parser.add_argument("--limit", type=int, default=10, help="Number of records to fetch")
    args = parser.parse_args()

    if not args.project:
        print("Error: --project or GCP_PROJECT environment variable is required.")
        sys.exit(1)
    if not args.app:
        print("Error: --app or FIREBASE_APP_ID environment variable is required.")
        sys.exit(1)

    # Automatically set Google credentials if not set
    # Checks for a SA key at a conventional path, or falls back to gcloud ADC
